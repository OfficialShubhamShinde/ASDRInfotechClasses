...
<head>
    <meta charset="utf-8">
    <title>Enroll - ASDR Infotech</title>
...
<body style="font-family: 'Poppins', sans-serif;">
                    ...
                    <div class="collapse navbar-collapse justify-content-between" id="navbarCollapse">
                        <div class="navbar-nav py-0">
                            ...
                            <a href="contact" class="nav-item nav-link">Contact Us</a>
                                ...
                                <a href="gallery" class="nav-item nav-link">Events & Media</a>
                                <a href="dashbordEvents" class="nav-item nav-link active">Dashbord</a>
                                ';
                            }
                            ?>
                        </div>
    ...
    <div class="sidebarMenu">
        ...
        <div class="w3-sidebar w3-bar-block w3-border-right" style="display:none" id="mySidebar">
            <button onclick="w3_close()" class="w3-bar-item w3-large" style="justify-items: right;"> &times;</button>
            <a href="dashbordEvents" class="w3-bar-item w3-button">Events</a>
            <a href="dashbordEntollCourses" class="w3-bar-item w3-button">Enroll Courses</a>
            <a href="dashbordNewslatter" class="w3-bar-item w3-button">Newsletter</a>
            <a href="dashbordManageCoupon" class="w3-bar-item w3-button">Manage Coupon</a>
            <a href="dashbordInternshipApplication" class="w3-bar-item w3-button">Internship Applications</a>
            <a href="dashbordContact" class="w3-bar-item w3-button">Contact</a>
        </div>
        ...
        <div class="container">
            <!-- Header Start -->
            <center>
            <h2 class="text-primary text-uppercase mb-3" style="letter-spacing: 5px;">Enroll Courses</h2>
            </center>
            <br>
            <!-- Header End -->
            <?php
            $SelFetch = "select * from payment_status";
            $querySelFetch = mysqli_query($conn, $SelFetch);

            if (mysqli_num_rows($querySelFetch) > 0 ){
                ?>
                ...
                <table class="table table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th scope="col">Enroll Course Name</th>
                    ...
                    <tbody>
                        ...
                        while ($row = mysqli_fetch_assoc($querySelFetch)) {
                            // $id  = $row['id '];
                            $payment_id = $row['payment_id'];

                            $student_id = $row['student_id'];
                            $selectStudentDetails = "select * from users where user_id = $student_id";
                            $querySelectStudentDetails = mysqli_query($conn, $selectStudentDetails);
                            $rowselectStudentDetails = mysqli_fetch_assoc($querySelectStudentDetails);
                            $sId = $rowselectStudentDetails['user_id'];
                            $SName = $rowselectStudentDetails['name'];
                            $SEmail = $rowselectStudentDetails['email'];

                            $course_id = $row['course_id'];
                            $selectCourseDetails = "Select * from courses where course_id = $course_id";
                            $querySelectCourseDetails = mysqli_query($conn, $selectCourseDetails);
                            $rowSelectCourseDetails = mysqli_fetch_assoc($querySelectCourseDetails);
                            $CName = $rowSelectCourseDetails['course_name'];

                            $gen_paymentid = $row['gen_paymentid'];
                            $payment_status = $row['payment_status'];
                            $payment_datetime = $row['payment_datetime'];

                            // $payment_datetime = date_format(date_create($datetime), 'h:i A');
                            // echo $formattedTime;
                        
                            $formattedDate = date_format(date_create($payment_datetime), 'Y/m/d');
                            // echo $formattedDate;
                            ...
                            <tr>
                                ...
                                <td>' . $formattedDate . '</td>
                                <td>' . $gen_paymentid . '</td>
                                <td><a href="printReceipt.php?sname=' . $SName . '&cname=' . $CName . '&cid=' . $course_id . '&sId=' . $sId . '"><button type="button" class="btn btn-primary btn-sm">Print</button></a></td>
                            </tr>
                            ';
                        }
                        ?>
            ...
            <?php
                ...
                echo '
                <center><h3>Currently no any enrollment available</h3></center>
                ';
    ...
    <script>
        $(document).ready(function () {
            // Update status when status element is clicked
            $('.status').on('click', function (e) {
                ...
                $.ajax({
                    url: 'update_status.php', // Replace with the actual file to handle the update
                    method: 'POST',
                    data: { course_id: courseId, status: newStatus },
