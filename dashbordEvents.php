...
<head>
    <meta charset="utf-8">
    <title>Events - ASDR Infotech</title>
...
<body style="font-family: 'Poppins', sans-serif;">
    ...
    <div class="sidebarMenu">
        ...
        <div class="container">
            <center>
                <h2 class="text-primary text-uppercase mb-3" style="letter-spacing: 5px;">EVENTS</h2>
            </center>
            <br>

            <?php
            $SelectForEventDetail = "select * from events";
            $querySelectForEventDetail = mysqli_query($conn, $SelectForEventDetail);

            $SelectForRejesteredDetail = "select * from registration";
            $querySelectForRejesteredDetail = mysqli_query($conn, $SelectForRejesteredDetail);


            if ( mysqli_num_rows($querySelectForRejesteredDetail) > 0) {
                ?>
                <div class="table-responsive" style="max-height: 1000px; overflow-y: auto; ">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            while ($row = mysqli_fetch_assoc($querySelectForRejesteredDetail)) {
                                $stu_name = $row['name'];
                                $event_id = $row['event_id'];
                                $stu_email = $row['email'];
                                $stu_contactno = $row['contactno'];
                                $stu_qualification = $row['qualification'];
                                $stu_experience = $row['experience'];
                                $stu_expectation = $row['expectation'];
                                $stu_regestered_on = $row['regestered_on'];

                                // for event name time mode etc:
                                $select = "select * from events where event_id = $event_id";
                                $query = mysqli_query($conn, $select);
                                $myRow = mysqli_fetch_assoc($query);
                                $eventTitle = $myRow['etitle'];
                                $edate = $myRow['edate'];
                                $eday = $myRow['eday'];
                                $emode = $myRow['emode'];
                                $estarttime = $myRow['estarttime'];

                                $formattedestarttime = date_format(date_create($estarttime), 'h:iA');
                                $formattededate = date_format(date_create($edate), 'Y/m/d');


                                echo '
                            <tr>
                                <td>' . ($eventTitle ? $eventTitle : 'NA') . '</td>
                                <td>' . $formattededate . '</td>
                                <td>' . $emode . '</td>
                                <td>' . $formattedestarttime . '</td>
                                <td>' . $stu_name . '</td>
                                <td>' . $stu_email . '</td>
                                <td>' . $stu_contactno . '</td>
                                <td>' . $stu_qualification . '</td>
                                <td>' . $stu_experience . '</td>
                                <td>' . $stu_expectation . '</td>
                                <td>' . $stu_regestered_on . '</td>
                                <td><a href="mailto:' . $stu_email . '"><button type="button" class="btn btn-primary btn-sm">Message</button></a></td>
                ...
                echo '
                <center><h3>Currently no any event registration available</h3></center>
                ';
            }
            ?>
